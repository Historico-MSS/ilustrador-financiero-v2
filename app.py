...
def style_money_pct_table(df: pd.DataFrame, money_cols=None, pct_cols=None):
    ...
    styled = out.style.map(color_valores, subset=subset)
    return styled
...
def limpiar_cambios(raw_cambios: list, fecha_inicio: pd.Timestamp) -> list:
    ...
    # quitar duplicados por fecha, dejando el último
    dedup = {}
    for c in validos:
        dedup[c["fecha"]] = c
...
# =========================================================
# CONSTRUIR SEGMENTOS DE ESTRATEGIA
# =========================================================
...
# =========================================================
# PORTAFOLIO CON CAMBIOS Y CONTINUIDAD REAL
# =========================================================
...
# =========================================================
# EVOLUCIÓN POR FONDO A LO LARGO DE LOS TRAMOS
# =========================================================
...
def generar_pdf_premium(
    ...
    with PdfPages(buffer) as pdf:
        # Página 1: resumen + gráfico principal
        ...
        ax31 = fig3.add_axes([0.08, 0.05, 0.34, 0.28])
        ax32 = fig3.add_axes([0.58, 0.05, 0.34, 0.28])
        ...
        ax4 = fig4.add_axes([0.08, 0.08, 0.84, 0.48])
...
if fondos_sel and total_inicial == 100:
    ...
    # construir portafolio con cambios
    ...
    if not df_res_snap.empty:
        ...
        if not estado_fecha.empty:
            ...
            st.info("No hay detalle por fondo para esa fecha.")
    else:
        st.info("No hay resultados para esa fecha.")
